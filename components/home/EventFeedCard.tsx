...
import { cancelEvent, EVENT_LIFECYCLE_LABELS, type EventResponse, type EventLifecycle } from "@/lib/events";
...
const getNextEventTimeBoundary = (event: Pick<EventResponse, "scheduledAt" | "endAt">, nowMs: number): number | null => {
  const boundaries = [event.scheduledAt, event.endAt]
...
function EventFeedCard({ event, headerLabel, repostCaption, taggedFriendNames = [], onRepostSuccess, onEventCancelled, onSaveChange, onHostBlocked, onHostFollowChange, embedded = false }: Props) {
  ...
  const eventLifecycle: EventLifecycle | null = event.lifecycle;
  const eventBadgeLabel = eventLifecycle ? EVENT_LIFECYCLE_LABELS[eventLifecycle] : null;
  const isLiveBadge = eventLifecycle === "live";
  ...
  useEffect(() => {
    ...
    const scheduleNextBoundary = () => {
      ...
      const nextBoundary = getNextEventTimeBoundary({
    ...
    // This timer supports the existing host-side end-time business behavior.
    // Lifecycle display is supplied by the backend in `event.lifecycle` and
    // intentionally does not depend on this client clock.
    scheduleNextBoundary();
    ...
    >
      ...
      <View style={styles.imageContainer}>
        ...
        <View style={styles.statusBadgeGroup} pointerEvents="none">
          {eventLifecycle ? <Animated.View
            style={[
              styles.statusBadge,
              eventLifecycle === "live" && styles.liveStatusBadge,
              eventLifecycle === "upcoming" && styles.upcomingStatusBadge,
              eventLifecycle === "starting_soon" && styles.startingSoonStatusBadge,
              eventLifecycle === "ended" && styles.endedStatusBadge,
          ...
          >
            <View style={styles.statusDotShell}>
              ...
              <View
                style={[
                  styles.statusDot,
                  eventLifecycle === "live" && { backgroundColor: colors.danger },
                  eventLifecycle === "upcoming" && styles.upcomingStatusDot,
                  eventLifecycle === "starting_soon" && styles.startingSoonStatusDot,
                  eventLifecycle === "ended" && styles.endedStatusDot,
                ]}
              />
            </View>
            <Text
              style={[
                styles.statusText,
                eventLifecycle === "live" && { color: colors.danger },
                eventLifecycle === "upcoming" && styles.upcomingStatusText,
                eventLifecycle === "starting_soon" && styles.startingSoonStatusText,
                eventLifecycle === "ended" && styles.endedStatusText,
              ]}
            >
              {eventBadgeLabel}
            </Text>
