...
//
// This component is now kept mounted (hidden via style, never unmounted)
// by home.tsx once the Scenes tab has been visited once, instead of being
// torn down every time another tab is selected — so `events`/`isLoading`/
// `hasLoadedOnceRef` below already survive leaving and returning to Scenes
// on their own. `isActive` only exists to turn a RETURN visit into an
// actual background refresh (see the effect below); it does not gate what
// this component renders.
function ParticipatedWindowsList({ isActive = true }: { isActive?: boolean }) {
  ...
  // Tracks isActive across renders purely to detect a false→true transition
  // (the user switching INTO the Scenes tab).
  ...
  useEffect(() => {
    // A tab switch INTO Scenes (isActive flips false → true) gets its own
    // fresh, background request — same "fresh data on every tab switch"
    // requirement as Discover/Friends. The very first activation is already
    // covered by the focus effect above (whichever fires first on mount),
    // so this only ever fires for a RETURN visit.
    if (shouldRefreshParticipatedWindowsOnActivate(isActive, wasActiveRef.current, hasLoadedOnceRef.current)) {
      void load(false);
    }
  ...
  // A background refresh failure (isLoading stayed false) must never
  // replace already-cached content with the blocking error view — only
  // show it when there's nothing cached to fall back to.
  if (shouldShowParticipatedWindowsBlockingError(loadError, events.length)) {
    return (
