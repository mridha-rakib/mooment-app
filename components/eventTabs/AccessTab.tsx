...
const AccessTab = ({
  ...
  const ageAdmissionNotice = getEventAgeAdmissionNotice(ageRestriction);
  ...
  const renderTickets = () => {
    ...
    return (
      <View style={{ marginTop: 20 }}>
        ...
        {!isDraftPreview && ((!isLocked && !isPrivate) || hasAcceptedRequest || (isPrivate && (isHostMode || isMember))) && (
        ...
        {isLocked && !hasAcceptedRequest && ageAdmissionNotice ? (
          <View style={[styles.alertBanner, { backgroundColor: isDark ? "#1C1718" : colors.card }]}>
            <Feather name="info" size={20} color="#E75737" />
            <Text style={[styles.alertText, { color: colors.textSecondary }]}>{ageAdmissionNotice}</Text>
          </View>
        ) : null}
