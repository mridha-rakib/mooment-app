...
function AuthSessionGate() {
  ...
  useEffect(() => {
    ...
    // If currently on splash root (no firstSegment), redirect immediately once restored
    if (!firstSegment) {
      router.replace(
        isAuthenticated
          ? ("/(tabs)/home" as any)
    ...
    const setup = async () => {
      try {
        ...
        const navigateFromNotification = (
          ...
          if (data.type === "ticket_buyer" && data.orderId && data.ticketId && data.ticketIndex) {
            void openPurchasedTicket(router, {
              orderId: data.orderId,
              ticketId: data.ticketId,
              ticketIndex: Number(data.ticketIndex),
            }).catch(() => {
              if (data.eventId) router.push({ pathname: "/event-screen/event", params: { eventId: data.eventId } } as any);
            });
          } else if (data.type === "dm" && data.conversationPartnerId) {
            router.push({
...
export default function RootLayout() {
  ...
  const isBootReady = fontsLoaded || Boolean(fontError) || timedOut;
