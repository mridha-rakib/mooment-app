...
import { getEventAgeAdmissionNotice } from "@/lib/eventAgeRestriction";
...
const EventCheckoutScreen = () => {
  ...
  // Keep Card available immediately. Apple Pay is added only after Stripe's
  // native capability check confirms this iPhone can use it.
    ...
    const checkApplePaySupport = async () => {
      try {
        ...
        if (isActive) {
          ...
          if (__DEV__) console.log("[ApplePay] supported:", supported);
        }
      } catch (error) {
        if (__DEV__) {
          console.warn(
            "[ApplePay] capability check failed:",
  ...
  const ageAdmissionNotice = getEventAgeAdmissionNotice(
    typeof params.ageRestriction === "string" ? params.ageRestriction : null,
  );
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const selectedPass = order.ticketPasses[0];
      if (selectedPass) {
        try {
          await openPurchasedTicket(router, {
            orderId: order.id,
            ticketId: selectedPass.ticketId,
            ticketIndex: selectedPass.ticketIndex,
          }, "replace");
        } catch {
          router.replace({ pathname: "/event-screen/wallet" });
        }
      } else {
        router.replace({ pathname: "/event-screen/wallet" });
      }
      ...
      >
        ...
        {ageAdmissionNotice ? (
          <View style={styles.noticeCard}>
            ...
            <Text style={styles.noticeText}>{ageAdmissionNotice}</Text>
          </View>
        ) : null}
