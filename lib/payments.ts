...
export type ScannedTicket = {
  ...
  holderName: string;
  usedAt: string;
  // EVT-010: the Event's admission age restriction ("all_ages" | "18_plus" |
  // "21_plus" | null) — informational only, never an age-verified state.
...
export type PurchasedTicketResult = {
...
export const getPurchasedTicket = async (identity: {
  orderId: string;
  ticketId: string;
  ticketIndex: number;
}): Promise<PurchasedTicketResult> => {
  const response = await api.get(
    `/payments/purchased-tickets/${encodeURIComponent(identity.orderId)}/${encodeURIComponent(identity.ticketId)}/${identity.ticketIndex}`,
  );
  const result = response.data?.data as PurchasedTicketResult | undefined;
  if (!result?.ticket || typeof result.remainingPurchasableQuantity !== "number") {
    throw new Error("Purchased ticket details are unavailable.");
  }
  return result;
};
