...
export type EventResponse = {
  ...
  hasReported?: boolean;
  status: EventStatus;
  // Backend-derived display lifecycle. Null intentionally preserves separate
  // draft/cancelled presentation rather than inventing a time-based fallback.
...
// EVT-014 reorder: IDs only, never a mutable ticket payload — see
// EventService.reorderEventTickets on the backend, which rejects anything but
// an exact permutation of the event's current ticket ID set.
export const reorderEventTickets = async (
  eventId: string,
  ticketIds: string[],
): Promise<EventResponse> => {
  const response = await api.patch(
    `/events/${encodeURIComponent(eventId)}/tickets/reorder`,
    { ticketIds },
  );
