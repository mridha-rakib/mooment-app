...
test("Apple Pay is only exposed after Stripe confirms iOS platform-pay support", () => {
  assert.match(checkoutSource, /Platform\.OS !== "ios"/);
  assert.match(checkoutSource, /isPlatformPaySupported\(\)/);
  assert.match(checkoutSource, /setIsApplePaySupported\(supported\)/);
  assert.match(renderSection, /showApplePay=\{isApplePaySupported\}/);
});
