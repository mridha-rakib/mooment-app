...
// Discover/Friends now cache independently (per-audience state), so this
// optimistic patch applies to BOTH cached audiences by id instead of one
// shared array — see updateFeedPostsForAllAudiences/updateFeedEventsForAllAudiences
// in app/lib/audienceFeedState.ts's usage pattern. The reconciliation logic
// itself (map by authorId, patch isFollowing) is unchanged.
test("handleAuthorFollowChange reconciles feedMomentPosts (pre-existing behavior, untouched)", () => {
  assert.match(
    homeSource,
    /updateFeedPostsForAllAudiences\(\(currentPosts\) => currentPosts\.map\(\(post\) => \(\s*post\.authorId === authorId \? \{ \.\.\.post, isFollowing \} : post\s*\)\)\);/,
  );
});
...
test("handleAuthorFollowChange also reconciles feedEvents (event host cards) for the same author id", () => {
  assert.match(
    homeSource,
    /updateFeedEventsForAllAudiences\(\(currentEvents\) => currentEvents\.map\(\(event\) => \(\s*event\.host && event\.host\.id === authorId\s*\? \{ \.\.\.event, host: \{ \.\.\.event\.host, isFollowing \} \}\s*: event\s*\)\)\);/,
  );
});
