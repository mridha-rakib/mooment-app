...
// ───────────────────── Canonical display lifecycle (J–L) ──────────────────

test("J/L — EventFeedCard badge consumes backend lifecycle, not persisted status", () => {
  assert.match(eventsLibSource, /EventLifecycle\s*=\s*"upcoming"\s*\|\s*"starting_soon"\s*\|\s*"live"\s*\|\s*"ended"/);
  assert.match(eventsLibSource, /lifecycle:\s*EventLifecycle \| null/);
  assert.match(eventFeedCardSource, /const eventLifecycle: EventLifecycle \| null = event\.lifecycle;/);
  assert.doesNotMatch(eventFeedCardSource, /getEventBadgeStatus/);
