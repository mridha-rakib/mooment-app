...
test("Public/Locked drafts and all published hosts reach the privacy pill", () => {
  assert.match(
    renderHeaderSection,
    /isHostMode &&\s*\(!isDraftPreview \|\| event\?\.privacy !== "private"\) &&\s*!isEventCompleted &&\s*!isEventCancelled/,
  );
});

test("Completed/cancelled events still hide the selector", () => {
  assert.match(renderHeaderSection, /!isEventCompleted &&\s*!isEventCancelled/);
});
...
test("draft dropdown keeps existing Public/Locked choices; published dropdown adds Private", () => {
  ...
  assert.match(privacyDropdownSection, /onPress=\{\(\) => handlePrivacyChange\("private"\)\}/);
  assert.match(privacyDropdownSection, /\{!isDraftPreview && \(/);
