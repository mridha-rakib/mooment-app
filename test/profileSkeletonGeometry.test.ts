...
test("ProfileStatsRowSkeleton renders exactly four columns, matching Events/Reviews/Followers/Following", () => {
...
test("ProfileHeader's real avatar/stat props (uri, name, stats.events/reviews/followers/following) are unchanged", () => {
  assert.match(headerSource, /uri=\{avatar\}/);
  assert.match(headerSource, /\{stats\.events\}/);
  assert.match(headerSource, /\{stats\.reviews\}/);
  assert.match(headerSource, /\{stats\.followers\}/);
  assert.match(headerSource, /\{stats\.following\}/);
});
