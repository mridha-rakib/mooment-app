...
test("feed treats the first render as loading so a skeleton shows immediately", () => {
  // Discover's initial per-audience cache starts isInitialLoading: true (no
  // useState(true) flag anymore — see app/lib/audienceFeedState.ts — but the
  // cold-start guarantee is the same: conceptually loading from the very
  // first render, before useFocusEffect's loadFeed runs a beat later).
  assert.match(
    homeSource,
  ...
  // Now keyed on isFeedInitialLoading specifically (not the combined
  // isFeedLoading), so a background tab-switch/focus refresh of an
  // already-loaded tab never brings the skeleton back.
  assert.match(
    homeSource,
    /shouldShowFeedSkeleton = selectedType === 'Feed' && !hasFeedLoadedOnce && isFeedInitialLoading && feedItems\.length === 0 && !isRefreshing/,
  );
