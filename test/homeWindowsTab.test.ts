...
test("HomeTabsRow (the selector) is rendered once, before the Feed-vs-Windows content switch, so it never disappears when Windows is selected", () => {
  ...
  // The Windows/Feed content switch is now a single `{selectedType === 'Feed'
  // ? (` block that renders both the (possibly hidden) ParticipatedWindowsList
  // and the (possibly hidden) FlatList side by side — `hasEverVisitedWindows`
  // is the first marker unique to that block (see ParticipatedWindowsList's
  // `isActive` wiring), distinguishing it from HomeTabsRow's own, earlier
  // `{selectedType === 'Feed' ? (` gate.
  const windowsBranchIndex = homeSource.indexOf("hasEverVisitedWindows ? (");
  assert.notEqual(tabsRowIndex, -1, "home.tsx should render HomeTabsRow");
  assert.notEqual(windowsBranchIndex, -1, "home.tsx should branch on hasEverVisitedWindows");
  assert.ok(
...
// 2 & 3 — Discover/Friends still exclusively own feedAudience/loadFeed;
// "windows" never touches either. handleAudienceChange's BODY intentionally
// changed as part of the tab-switch cache fix (it no longer wipes that
// audience's cached posts/events/reposts before fetching — see
// app/lib/audienceFeedState.ts) — its CONTRACT (guarded to feedAudience,
// only ever called for discover/friends) did not.
test("Discover/Friends still flow through handleAudienceChange, now cache-preserving instead of the old eager-clear", () => {
  ...
  assert.match(handleAudienceChangeFn, /void loadFeed\(audience\);/);
  // The old wipe is gone — no clearing of that audience's cached arrays.
  assert.doesNotMatch(handleAudienceChangeFn, /beginAudienceTransition/);
  assert.doesNotMatch(handleAudienceChangeFn, /posts: \[\]/);
  assert.doesNotMatch(handleAudienceChangeFn, /windows/);
});

test("selecting Discover or Friends still routes through handleAudienceChange; only \"windows\" is new", () => {
  ...
  assert.match(
    handleHomeAudienceChangeFn,
    /if \(tab === "windows"\) \{\s*setHasEverVisitedWindows\(true\);\s*\} else \{\s*handleAudienceChange\(tab\);/,
  );
});
