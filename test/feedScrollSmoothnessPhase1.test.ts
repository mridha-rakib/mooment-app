...
// Discover/Friends now cache independently (per-audience state — see
// app/lib/audienceFeedState.ts), so the scroll-deferral / stale-request
// guards below are keyed per audience: feedRequestIdRef and
// pendingFeedRefreshCommitRef are Record<FeedAudience, ...>, and deferral
// itself only ever applies to the audience currently on screen (an inactive
// audience's commit can't visually disrupt anything, so it always applies
// immediately regardless of scroll state — see shouldDeferAudienceFeedCommit).
test("feed refresh while scrolling does not immediately replace visible arrays", () => {
  ...
  assert.match(homeSource, /const isActiveAudience = audience === feedAudienceRef\.current;/);
  assert.match(homeSource, /if \(shouldDeferAudienceFeedCommit\(isActiveAudience, isFeedScrollingRef\.current, nextCommit\.hasAnyFreshData\)\) \{/);
  assert.match(homeSource, /pendingFeedRefreshCommitRef\.current\[audience\] = nextCommit;/);
});
...
test("latest pending refresh is applied once scroll becomes idle", () => {
  ...
  assert.match(homeSource, /pendingFeedRefreshCommitRef\.current\[audience\] = null;\s*applyFeedRefreshCommit\(audience, pendingCommit\);/);
});
...
test("multiple refreshes while scrolling use latest-result-wins semantics", () => {
  ...
  assert.match(homeSource, /const requestId = \+\+feedRequestIdRef\.current\[audience\];/);
  assert.match(homeSource, /if \(!isLatestFeedRefreshCommit\(pendingCommit, feedRequestIdRef\.current\[audience\]\)\) \{/);
});
...
test("feed refresh commit replaces arrays and does not append duplicate items", () => {
  ...
  // The merge itself (full replace per resolved source, never prepend/
  // append — see mergeAudienceFeedCommit) now lives in a pure, directly
  // tested helper (app/test/audienceFeedState.test.ts: "a successful commit
  // fully replaces every resolved source — never merges/prepends").
  assert.match(applyCommitBody, /mergeAudienceFeedCommit\(current\[audience\], commit\)/);
